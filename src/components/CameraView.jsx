...
const CameraView = () => {
  ...
  // Default app configuration for demo
  const [appConfig] = useState({
    businessName: 'Demo Fahrschule',
    primaryColor: '#1E40AF',
  ...
  }, [appConfig.textOverlay, hasPermission])
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setLocationError(
          error.code === 1 ? "Standortzugriff verweigert" : "Standortermittlung fehlgeschlagen"
        )
      }
    )
  }
  ...
  const startCamera = async () => {
    if (!hasPermission('camera.use')) {
      alert('Sie haben keine Berechtigung, die Kamera zu verwenden')
      return
    }
  ...
  const capturePhoto = async () => {
    ...
    if (!hasPermission('photo.save')) {
      alert('Sie haben keine Berechtigung, Fotos zu speichern')
      return
    }
  ...
  const shareImage = () => {
    if (!hasPermission('photo.share')) {
      alert('Sie haben keine Berechtigung, Fotos zu teilen')
      return
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600">
            Sie haben keine Berechtigung, die Kamera zu verwenden.
          </p>
        </div>
      </div>
    )
              ...
              {!capturedImage ? (
                <>
                  <motion.button
                    ...
                    className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-full font-semibold flex items-center space-x-2 shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
